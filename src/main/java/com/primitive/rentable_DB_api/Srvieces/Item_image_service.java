            ...
            if (!file[i].isEmpty()) {
                ...
                try{
                    ...
                    if(rs2.next()){
                        ...
                        con.createStatement().execute(String.format("insert item_images (index, owners_user_index, path, uploaded_date_time) values(%d,'%s','%s','%s');",idx+1,related_item_index,DBPath,now_date_time));
                    }
                    con.close();
                }catch (Exception e){
                    e.printStackTrace();
