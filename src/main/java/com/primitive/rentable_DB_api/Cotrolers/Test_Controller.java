...
public class Test_Controller {
    ...
    public String testController(){
        try {
            ...
            //업로드 된 아이템 테이블
            con.createStatement().execute("create table if not exists items_uploaded (idx int, owners_user_id varchar(20), uploaded_date_time varchar(20), title varchar(80), content varchar(4000), appended_image_count int, is_deleted boolean, reservation_table_name varchar(50), comment_table_name varchar(50), reviews_table_name varchar(50));");
            //모든 사용자 테이블
            ...
            con.createStatement().execute("create table if not exists profile_images (idx int, owners_user_id varchar(20), Path varchar(200), uploaded_date_time varchar(20));");
            ...
            ResultSet rs2 = con.createStatement().executeQuery("select idx from items order by desc idx limit 1");
            if (rs2.next()){
